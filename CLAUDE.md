...
- Env: `RFQ_FROM_ADDRESS` (sending mailbox, default `rob@dearborndenim.com`), `RFQ_FILES_DIR`, `PUBLIC_BASE_URL`/`BASE_URL`, `RFQ_BRAND_ID` (default `dearborn-denim`), plus the existing `PRODUCT_DEV_URL`/`PRODUCT_DEV_KEY`.
...
- Built-in `email` hand (spec §12.4): the ONLY way a message leaves Robert's Outlook mailbox. File `action_payload: { hand: 'email', method: 'POST', path: '/send', body: { to, cc?, subject, text, html?, attachments?: [{url, name}], rfq_id? } }` — `to`/`cc` take one address or an array (≤10), `subject` ≤120… ≤300 chars, `text` ≤20 000, `html` optional (sent instead of `text` when present), `attachments` ≤5 public http(s) URLs fetched at send time (design-module `/files/...` or our own `/files/rfq/...`), each ≤4 MB; one that 404s or is oversized is skipped and named in the notify line rather than failing the send. No brand config entry is needed (a brand may register its own `hands.email` to override). There is **no HTTP route that sends mail** — a send is always a proposal execution, so the trust ledger (level 1 → Robert taps Approve) has already decided. The Telegram card shows recipient, subject, body and attachment count instead of `action_type → hand/path`. Execution goes out through Microsoft Graph `sendMail` from `RFQ_FROM_ADDRESS` (default `rob@dearborndenim.com`), returns `{ok:true, notify:"Sent to <to>: <subject>"}` (the executor forwards `notify`), and records one `rfq_messages` row per recipient. A Graph 4xx marks the proposal `failed` with the status on `execution_result`.
- `rfq_messages` table (`src/db/spine-schema.ts`, queries in `src/db/rfq-queries.ts`): `{rfq_id, vendor_email, vendor_domain, subject, graph_message_id, sent_at, proposal_id, brand_id, intents}`. `rfq_id` comes from `body.rfq_id`, then `evidence.rfq_id`, then the `[DD-RFQ-<id>]` subject tag; `intents` is the fabric-intent CSV copied from the proposal's `evidence` at send time, and is what the reply's quotes get filed against. Graph's `sendMail` answers 202 with no message id, so `graph_message_id` holds the Graph correlation id (`request-id`, else the `client-request-id` we sent) — reply correlation never depends on it.
