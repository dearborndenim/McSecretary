...
- **Recognition** (`matchRfqReply`): the `[DD-RFQ-<id>]` tag in the subject or body preview wins (it survives a reply from a different mailbox); otherwise the sender's domain matching an `rfq_messages` row sent within **60 days**. Anything else takes the ordinary Haiku classifier path.
...
- Env: `RFQ_MAILBOX` (the Graph mailbox that sends/receives, default `OUTLOOK_USER_EMAIL_1` else `rob@dearborndenim.com`), `RFQ_FROM_ADDRESS` (the address shown as the sender — an alias of `RFQ_MAILBOX`; defaults to `RFQ_MAILBOX` itself, i.e. no alias), `RFQ_FILES_DIR`, `PUBLIC_BASE_URL`/`BASE_URL`, `RFQ_BRAND_ID` (default `dearborn-denim`), plus the existing `PRODUCT_DEV_URL`/`PRODUCT_DEV_KEY`.
