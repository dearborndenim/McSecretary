...
- `src/email/rfq-intake.ts` — RFQ reply recognition, the extraction schema/prompt, the vendor-quote bodies, the notes card
...
- `src/db/rfq-queries.ts` — `rfq_messages` writes and the two reply lookups
...
- **Recognition** (`matchRfqReply`, called from `src/triage.ts` before the classifier): the `[DD-RFQ-<id>]` tag in the subject or body preview wins (it survives a reply from a different mailbox); otherwise the sender's domain matching an `rfq_messages` row sent within **60 days**. Anything else takes the ordinary Haiku classifier path.
